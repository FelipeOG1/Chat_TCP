...
static void init_room(Room *room,AddRoom *add_room_msg,int client_sockfd){
  ...
  strlcpy(room->room_name,add_room_msg->room_name,room->room_name_len + 1);//+1 pos add '\0'
...
void add_room(Rooms *rooms ,AddRoom * add_room,int sockfd){
  ...
  strlcpy(rooms->room_names[rooms->n_rooms],room.room_name,current_room_name_len + 1);
  rooms->n_rooms+=1;
